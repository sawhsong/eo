...
public class CommonLookupSelectbox extends TaglibSupport {
	...
	public int doStartTag() {
		try {
			...
			for (int i=0; i<ds.getRowCnt(); i++) {
				...
				if (CommonUtil.equals(ds.getValue(i, "LOOKUP_CODE"), selectedValue)) {selected = " selected";}
				if (CommonUtil.containsIgnoreCase(disabledValue, ds.getValue(i, "LOOKUP_CODE"))) {disabledOption = " disabled";}

				html.append("<option value=\""+ds.getValue(i, "LOOKUP_CODE")+"\""+selected+disabledOption+">"+ds.getValue(i, "MEANING")+"</option>\n");
			}
